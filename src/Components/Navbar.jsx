...
const Location=useLocation();
  ...
  return (
    <motion.nav
      initial={{ y: -100 }}
      animate={{ y: 0 }}
      className='fixed top-0 left-0 right-0 z-50 h-16 flex items-center justify-between px-8 md:px-16 bg-black/40 backdrop-blur-md border-b border-white/5'
    >
      {/* 1. LOGO */}
      <div className='flex items-center font-display'>
        <span className='font-bold text-xl bg-gradient-to-r from-neon-blue to-electric-purple bg-clip-text text-transparent drop-shadow-[0_0_10px_rgba(0,163,255,0.3)]'>
          Tech
        </span>      
        <span className='text-soft-white font-bold text-xl ml-2'>Nova</span>
...
{Location.pathname === '/categories' && (
      ...
      <div className='flex items-center gap-3'>
        {/*login button*/}
        <button className='hidden md:flex items-center gap-2 px-5 py-2 rounded-full border border-white/10 bg-white/5 text-soft-white hover:bg-white/10 hover:border-white/20 transition-all cursor-pointer group'>
...
<AnimatePresence>
        {isOpen && (
          ...
          >
            ...
            <Link to="/categories" onClick={() => setIsOpen(false)} className={` ${Location.pathname==='/products'? 'nav-item-active':'nav-item-inactive'}  w-full text-center py-6 text-xs font-bold uppercase tracking-widest `}>
              Categories
            </Link>


<div className="px-8 mt-4 pt-6 border-t border-white/10">
        <button className="w-full flex items-center justify-center gap-2 px-5 py-3 rounded-full border border-white/10 bg-white/5 text-soft-white active:bg-white/20 transition-all">
